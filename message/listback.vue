    ...
    <div class="message">
        <scroller
            ...
            <ul v-if="info.length > 0" id="ul">
                <li v-for="(x,k) in info" @click="getContent(x.rid, x.mid, x)" >
                    <mt-cell-swipe
                      :right="[
                        {
                          content: '删除',
        ...
        methods: {
            ...
            getContent : function (rid, mid, x) {
                this.Http('message/content',{
                    ...
                    // window.location.href = '/#/message/content?rid=' + rid + '&mid=' + mid + '&c=' +JSON.stringify(x);
                })
            },
            getIcon:function () {
                let that = this;
